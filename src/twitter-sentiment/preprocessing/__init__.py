...
### RETURN THE STR OF STEMS from a tokenized tweet
def stem_one(text):
	"""
	Function to stem a cleaned tweet
	Inputs: text -- cleaned tweet str
			stemmer -- An NLTK
	Outputs: words -- a str of filtered and tokenized words
	"""
	# sanity check
	if (type(text) != str):
		return ''
	...
	words = [word if (word[0:2] != '__') else word.lower() \
				...
				if ((len(word) >= 3) and (word.startswith("__", 0, 2) == False))]
